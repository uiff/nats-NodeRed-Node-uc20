...
function mapAccessTypeToHashString(ac) {
    ...
    // Node-RED might have 'READWRITE', 'READ_WRITE', 'readwrite', etc.
    const normalized = String(ac).toUpperCase();
    if (normalized === 'READWRITE' || normalized === 'READ_WRITE') return 'read-write';
    return 'read-only';
}
...
function computeFingerprint(defs) {
    ...
    for (const def of [...defs].sort((a, b) => a.id - b.id)) {
        ...
        // Ensure input strings are handled case-insensitively where possible
        const dtStr = mapDataTypeToHashString(String(def.dataType).toUpperCase());
